...
plot(bodyfat$BODYFAT, type = "l", col = "red", main = "Real Values vs Predicted Values")
lines(model2.pre$fit, col = "blue")
legend("topleft", legend=c("real values", "predicted values"),col=c("red", "blue"), lty=1, cex=0.8)
library(ggplot2)
options(repr.plot.width=8, repr.plot.height=2);ggplot()+geom_line(aes(c(1:249,1:249),c(model2.pre$fit,bodyfat$BODYFAT),color=as.factor(rep(c("Predicted value","True value"),each=249))))+
